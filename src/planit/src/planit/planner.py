...
class Planner:
    ...
    def __init__(self, gripper_width=0.042, is_sim=False, commander_args=[]):
        ...
        self.grasps = Grasps(self.scene, gripper_width)

    def testScene(self):
        self.scene.add_box(
            'cafe_table.link', list_to_pose_stamped([1.0, 0.5, 0.78, 0, 0, 0], 'world'),
            (0.2, 0.4, 0.4)
        )
        self.scene.add_box(
            'cafe_table_clone.link',
            list_to_pose_stamped([0.5, 1.0, 0.78, 0, 0, 0], 'world'), (0.4, 0.2, 0.4)
        )
        self.scene.add_box(
            'wood_block_10_2_1cm.link',
            list_to_pose_stamped([1.0, 0.5, 1.03, 0, 0, 0], 'world'), (0.01, 0.02, 0.1)
        )
    ...
    def plan_ee_poses(
        self, ee_poses=[[0, 0, 0, 0, 0, 0]], constraints=None, group_name="left_arm"
    ):
        move_group = moveit_commander.MoveGroupCommander(group_name)
        move_group.set_num_planning_attempts(50)
    ...
    def wait_for_state_update(
        self, obj_name, box_is_known=False, box_is_attached=False, timeout=4
    ):
    ...
    def attach(
        self, obj_name, grasping_group='left_hand', group_name="left_arm", timeout=4
    ):
        move_group = moveit_commander.MoveGroupCommander(group_name)
