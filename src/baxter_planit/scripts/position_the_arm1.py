...
def position_the_arm():
    ...
    direction = [0,1,0]
    length = -0.3
    plan, planning_time = planner.plan_line_traj(direction = direction, magnitude = length, group_name="right_arm", avoid_collisions=False)
    planner.execute(plan, group_name="right_arm")
    # PH.straight_movement([0,1,0], 0.3)
