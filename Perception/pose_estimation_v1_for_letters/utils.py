...
def camera_to_robot(pt, matrix):
    camera_pt = np.array([pt[0], pt[1], 1])
    robot_pt = np.dot(matrix, camera_pt)
    robot_pt = np.array([robot_pt[0], robot_pt[1]]) / robot_pt[2]
    return robot_pt


def robot_to_camera(self, pt, matrix):
    robot_pt = np.array([pt[0], pt[1], 1])
    camera_pt = np.dot(matrix, robot_pt)
...
def get_pointcloud(color_img, depth_img, camera_intrinsics):
    ...
    # print('max', np.max(cam_pts_z))
    # print('min', np.min(cam_pts_z))

    return cam_pts, rgb_pts


def get_2D_pointcloud(
    color_img,
    camera_to_robot_matrix,
    robot_to_camera_matrix,
):

    # Get depth image size
    im_h = color_img.shape[0]
    im_w = color_img.shape[1]

    rgb_pts = []
    pix_x, pix_y = np.meshgrid(np.linspace(0, im_w - 1, im_w), np.linspace(0, im_h - 1, im_h))
    pix_x.shape = (im_h * im_w, 1)
    pix_y.shape = (im_h * im_w, 1)
    world_pts_x = (
        camera_to_robot_matrix[0, 0] * pix_x + camera_to_robot_matrix[0, 1] * pix_y + camera_to_robot_matrix[0, 2]
    )
    world_pts_y = (
        camera_to_robot_matrix[1, 0] * pix_x + camera_to_robot_matrix[1, 1] * pix_y + camera_to_robot_matrix[1, 2]
    )
    world_pts_scale = (
        camera_to_robot_matrix[2, 0] * pix_x + camera_to_robot_matrix[2, 1] * pix_y + camera_to_robot_matrix[2, 2]
    )
    world_pts_x = world_pts_x / world_pts_scale
    world_pts_y = world_pts_y / world_pts_scale
    rgb_pts_r = color_img[:, :, 0]
    rgb_pts_g = color_img[:, :, 1]
    rgb_pts_b = color_img[:, :, 2]
    rgb_pts_r.shape = (im_h * im_w, 1)
    ...
    world_pts = np.concatenate((world_pts_x, world_pts_y), axis=1)
    rgb_pts = np.concatenate((rgb_pts_r, rgb_pts_g, rgb_pts_b), axis=1)

    # print("computed", world_pts[0, 0])
    # world_pts1 = []
    # rgb_pts1 = []
    # for h in range(im_h):
    #     for w in range(im_w):
    #         world_pts1.append(camera_to_robot((w, h), camera_to_robot_matrix))
    #         rgb_pts1.append((color_img[h, w, 0], color_img[h, w, 1], color_img[h, w, 2]))

    # print("real")

    # print(world_pts1[0][0])
...
def get_heightmap(
    ...
    heightmap_size = np.array(
        (
            math.ceil((workspace_limits[1][1] - workspace_limits[1][0]) / heightmap_resolution),
            math.ceil((workspace_limits[0][1] - workspace_limits[0][0]) / heightmap_resolution),
        )
    ).astype(int)
    ...
    # real_surface_pts = surface_pts[sort_z_ind]
    ...
    # # Create orthographic top-down-view RGB-D heightmaps
    # real_depth_heightmap = np.zeros(heightmap_size)
    # heightmap_pix_x = np.floor((surface_pts[:, 0] - workspace_limits[0][0]) / heightmap_resolution).astype(int)
    # heightmap_pix_y = np.floor((surface_pts[:, 1] - workspace_limits[1][0]) / heightmap_resolution).astype(int)
    # real_depth_heightmap[heightmap_pix_y, heightmap_pix_x] = surface_pts[:, 2]
    # z_bottom = workspace_limits[2][0]
    # real_depth_heightmap = real_depth_heightmap - z_bottom
    # real_depth_heightmap = real_depth_heightmap.astype(np.float32)

    # color_heightmap = cv2.medianBlur(color_heightmap, 3)
    # real_depth_heightmap = cv2.medianBlur(real_depth_heightmap, 3)

    # not using depth
    surface_pts, color_pts = get_2D_pointcloud(color_img, camera_to_robot_matrix, robot_to_camera_matrix)
    # Filter out surface points outside heightmap boundaries
    heightmap_valid_ind = np.logical_and(
        np.logical_and(
            np.logical_and(surface_pts[:, 0] >= workspace_limits[0][0], surface_pts[:, 0] < workspace_limits[0][1]),
...
def get_both_heightmaps(
    ...
    # Sort surface points by z value
    sort_z_ind = np.argsort(surface_pts[:, 2])
    real_surface_pts = surface_pts[sort_z_ind]  # the copy used for the depth heightmap without filtering
    surface_pts = surface_pts[sort_z_ind]
    color_pts = color_pts[sort_z_ind]
    ...
    # # compute color heightmap without using depth
    # surface_pts, color_pts = get_2D_pointcloud(color_img, camera_to_robot_matrix, robot_to_camera_matrix)
    # # Filter out surface points outside heightmap boundaries
    # heightmap_valid_ind = np.logical_and(
    #     np.logical_and(
    ...
    return color_heightmap, depth_heightmap, real_depth_heightmap


def get_heightmap_copy_3d_0825(color_img, depth_img, cam_intrinsics, cam_pose, workspace_limits, heightmap_resolution):
...
def get_pixel(depth_img, camera_intrinsics, cam_x, cam_y, cam_z):
    ...
    for i in range(im_w):
        for j in range(im_h):
            error = np.linalg.norm(
                np.array([cam_pts_x[j, i], cam_pts_y[j, i], depth_img[j, i]]) - np.array([cam_x, cam_y, cam_z])
            )
            if error < min_error:
                min_error = error
                best_fit = (i, j)
    print("pixel", best_fit)
    print("best", [cam_pts_x[j, i], cam_pts_y[j, i], depth_img[j, i]], [cam_x, cam_y, cam_z])
    return best_fit


def reverse_heightmap(
    pts, cam_intrinsics, cam_pose, workspace_limits, heightmap_resolution, depth_heightmap, depth_img, color_heightmap
):
    surface_pts = np.zeros((pts.shape[0], 3))
    surface_pts[:, 0] = pts[:, 0] * heightmap_resolution + workspace_limits[0][0]
    surface_pts[:, 1] = pts[:, 1] * heightmap_resolution + workspace_limits[1][0]
    surface_pts[:, 2] = depth_heightmap[pts[:, 1], pts[:, 0]] + workspace_limits[2][0]

    # 2D point of the camera
    camera_pts = np.transpose(
        np.dot(
            np.transpose(cam_pose[0:3, 0:3]),
            (np.transpose(surface_pts) - np.tile(cam_pose[0:3, 3:], (1, surface_pts.shape[0]))),
        )
    )
    # print('camera_pts', camera_pts)

    # print('intrin', cam_intrinsics)

    pixel_pts = np.zeros((camera_pts.shape[0], 2))
    pixel_pts[:, 0] = camera_pts[:, 0] * cam_intrinsics[0][0] / camera_pts[:, 2] + cam_intrinsics[0][2]
    pixel_pts[:, 1] = camera_pts[:, 1] * cam_intrinsics[1][1] / camera_pts[:, 2] + cam_intrinsics[1][2]

    # return [ get_pixel(depth_img, cam_intrinsics, p[0], p[1], p[2]) for p in camera_pts]
    # print('pix',pixel_pts)
    px_min = min(pixel_pts[:, 0])
    px_max = max(pixel_pts[:, 0])
    py_min = min(pixel_pts[:, 1])
    py_max = max(pixel_pts[:, 1])
    return [(int(round(px_min)), int(round(py_min))), (int(round(px_max)), int(round(py_max)))]
        ...
        for vis_col in range(4):
            ...
            if rotate_idx == best_pix_ind[0]:
                affordance_vis = cv2.circle(
                    affordance_vis, (int(best_pix_ind[2]), int(best_pix_ind[1])), 7, (0, 0, 255), 2
                )
...
def back_range(v, minx, maxx):
    return min(max(v, minx), maxx)
